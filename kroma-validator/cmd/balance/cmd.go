...
func Deposit(ctx *cli.Context) error {
	...
	if err = sendTransaction(ctx, valpoolAddr, txData, depositAmount); err != nil {
		return err
	}
...
func TryUnjail(ctx *cli.Context) error {
	valManagerAbi, err := bindings.ValidatorManagerMetaData.GetAbi()
	if err != nil {
		return fmt.Errorf("failed to get ValidatorManager ABI: %w", err)
	}

	txData, err := valManagerAbi.Pack("tryUnjail")
	if err != nil {
...
func sendTransaction(ctx *cli.Context, txTo common.Address, txData []byte, txValue uint64) error {
